...
bartvschorni.play();  },1000);
    ...
    tt.addEventListener('touchstart',()=>{fix(l,j,m);active[1] = true; cl();});
    tt.addEventListener('touchend',()=>{h = 1; person(); clearInterval(v); active[1] = false;});
    bb.addEventListener('touchstart',()=>{fix(v,j,m);active[2] = true; cl();});
    bb.addEventListener('touchend',()=>{h = 1; person(); clearInterval(l); active[2] = false;
    });
    
    lt.addEventListener('touchstart',()=>{fix(m,v,l);active[3] = true; cl();});
    lt.addEventListener('touchend',()=>{h = 1; person(); clearInterval(j); active[3] = false;});
    
    rr.addEventListener('touchstart',()=>{fix(v,l,j);active[4] = true; cl();});
    rr.addEventListener('touchend',()=>{h = 1; person(); clearInterval(m); active[4] = false;});
    sp.addEventListener('touchstart',()=>{
        h = 4; person(); active[0] = false; active[1] = false; active[2] = false; active[3] = false; bartspecial.style.top = (parseFloat(bartspecial.style.top)-10)+'vh';
    });
